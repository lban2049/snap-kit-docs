...
为确保 SnapKit 稳定运行，建议您的系统至少具备以下资源：

- **内存**: 最低 2GB，推荐 4GB 或更高
- **磁盘空间**: 至少 5GB 可用空间用于存储截图和数据
- **CPU**: 双核处理器，推荐四核或更高

> **提示**: 如果需要处理大量并发任务，建议增加内存和 CPU 资源。

## 安装方式

SnapKit 提供多种安装和部署方式，您可以根据需求选择：
...
您应该看到 SnapKit 的 Web 界面。

![SnapKit 界面截图](../sources/snap-kit/blocklets/snap-kit/screenshots/1.jpeg)
...
SnapKit 的所有 API 都需要访问密钥进行身份验证。

### 生成访问密钥

1. 访问 SnapKit 的 Web 界面
2. 进入设置页面
3. 点击"生成新的访问密钥"
4. 保存生成的密钥（密钥只会显示一次）
